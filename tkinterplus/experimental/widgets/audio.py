...
from ... import MaterialIcon, Tooltip
...
# TODO Create a way to handle updating the visual timer.
...
class Audio(Frame):
    ...
    def _total_time(self) -> str:
        ...
        if t_hours >= 0:
            return "%02d:%02d" % (t_minutes, t_seconds)
        return "%02d:%02d:%02d" % (t_hours, t_minutes, t_seconds)

    def _get_channel(self) -> int:
        name = self.winfo_name().replace("!audio", "")
        return 1 if name == "" else self.getint(name)

    def update_time(self, loop: bool = False, move: bool = True) -> None:
        ...
        if ms > -1:
            ...
            if loop and self.audio_state == "playing":
                self._loop = self.after(
                    self.update_timer, lambda: self.update_time(True)
                )
    ...
    def open(self, file) -> None:
        ...
        self.total_seconds.set(self.sound.get_length())
    ...
    def play(self, loops: int = 0, maxtime: int = 0, fade_ms: int = 0) -> None:
        ...
        mixer.Channel(self._get_channel()).play(self.sound, loops, maxtime, fade_ms)
        if self.audio_state == "stopped":
            self.update_event = self.after(
                self.update_timer, lambda: self.update_time(True)
            )
    ...
    def configure(self, **kw) -> Self:
        ...
        if "activebackground" in kw and kw["activebackground"] != None:
            self.activebackground = kw.pop("activebackground")
        ...
        if "activeforeground" in kw and kw["activeforeground"] != None:
            self.activeforeground = kw.pop("activeforeground")
        ...
        if "troughcolor" in kw and kw["troughcolor"] != None:
            self.troughcolor = kw.pop("troughcolor")

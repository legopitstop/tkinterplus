...
class Tween:
    def __init__(self, widget, prop):
        ...
        self.configure(widget=widget, prop=prop)

    def configure(self, **kw):
        if "widget" in kw and kw["widget"]:
            self.widget = kw["widget"]
        if "prop" in kw and kw["prop"]:
            self.prop = kw["prop"]
...
class fx:
    ...
    def __run_widget(self):
        for o in self.options:
            ...
            if o in number:
                self._widget_number(
                    o, int(str(self.start.get(o))), int(str(self.end.get(o)))
                )
            elif o in color:
                self._widget_color(o, self.start.get(o), self.end.get(o))
            elif o in special:
                self._widget_special(o, self.start.get(o), self.end.get(o))
            else:
    ...
    def _item_special(self, option: str, start: int, end: int):
        self._special(
            ...
            lambda kw, tag=self.TagOrId: self.widget.moveto(tag, **kw),
        )
    ...
    def __run_item(self):
        for o in self.options:
            ...
            if o in number:
                tween = Tween(
                    "item_number",
                    int(str(self.start.get(o))),
                    int(str(self.end.get(o))),
                )
            elif o in color:
                self._item_color(o, self.start.get(o), self.end.get(o))
            elif o in special:
        ...
        **properties,
        ...
        if self.TagOrId == None:  # Widget
            for prop in properties:
                match prop:
                    case "x":
                        item.start[prop] = self.widget.winfo_x()
                    case "y":
                        item.start[prop] = self.widget.winfo_y()
                    case _:
                        item.start[prop] = self.widget.cget(
                            prop

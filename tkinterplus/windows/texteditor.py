...
class TextEditor(tkinter.Toplevel):
    def __init__(self, master: tkinter.Tk = None):
        ...
        self.os = "Linux"
        if self.os == "Windows":
            self.app = WindowsEditor(self)
        elif self.os == "Linux":
            self.app = LinuxEditor(self)
        elif self.os == "Darwin":
            self.app = DarwinEditor(self)
        else:
            self.app = DefaultEditor(self)

    def exit(self) -> None:
        self.destroy()

    def open(self, fp: str, mode="r") -> None:
...
class WindowsEditor:
    def __init__(self, master):
        ...
        xscroll = tkinter.Scrollbar(
            self.m, orient=tkinter.HORIZONTAL, command=self.textarea.xview
        )
        ...
        context.add_separator()
        context.add_command(
            label="Select All", command=lambda: self.context("select_all")
        )
        context.add_separator()
        context.add_command(
            label="Search Internet...", command=lambda: self.context("search")
        )
        ...
        file.add_separator()
        file.add_command(
            label="Page Setup...", command=lambda: self.context("page_setup")
        )
        ...
        edit.add_command(
            label="Find Previous", command=lambda: self.context("find_previous")
        )
        ...
        zoom.add_command(
            label="Restore Default Zoom", command=lambda: self.context("restore_zoom")
        )
    ...
    def update(self) -> None:
        ...
        if self.STATUS_BAR.get():
            ...
            tkinter.Label(pos, text="Ln 14, Col 9").grid(
                row=0, column=0, sticky=tkinter.E
            )
    ...
    def context(self, id: str) -> bool:
        if id == "open":
            ...
            file = filedialog.asksaveasfilename(
                confirmoverwrite=True, defaultextension=".txt", parent=self.m
            )
            if file != "":
                self.m.open(file, "w")
...
class DefaultEditor:
    def __init__(self, master):
        ...
        self.textarea = ScrolledText(
            self.m, bd=0, width=69, height=23, undo=True, wrap=tkinter.NONE
        )
        self.textarea.grid(row=0, column=0, sticky="nesw")
        xscroll = tkinter.Scrollbar(
            self.m, orient=tkinter.HORIZONTAL, command=self.textarea.xview

...
__all__ = ["Sprite", "Spritesheet", "open", "new"]
...
class Sprite:
    def __init__(self, row: int, column: int, name: str = None, fp: str = None):
        ...
        self.fp = str(fp)
        if name == None:
            self.name = os.path.basename(self.fp)
...
class SpriteSheet:
    ...
    def open(self, fp: str, formats=None) -> None:
    ...
    def resize(self, row: int, column: int) -> None:
    ...
    def add_sprite(self, fp: str, name: str = None, formats=None) -> Sprite:
    ...
    def save(self, fp, format=None) -> None:
        data = {"width": self.size[1], "height": self.size[1], "textures": {}}

        for sprite in self.sprites:
            data["textures"][sprite.name] = [sprite.row, sprite.column]

        with builtins.open(fp + ".meta", "w") as e:
            e.write(json.dumps(data))
        return self.spritesheet.save(fp, format)

    def get_sprite(self, row: int, column: int) -> ImageFile.ImageFile:
        ...
        if row <= self.row and row > 0 and column <= self.column and column > 0:
            ...
            y = self.size[1] * (row - 1)
            return self.spritesheet.resize(
                self.size,
                Resampling.NEAREST,
    ...
    def get_sprite_name(self, name: str) -> ImageFile.ImageFile:
...
def open(fp, size, formats=None, max_columns=None) -> SpriteSheet:
    return SpriteSheet(size, max_columns).open(fp, formats)


def new(size, max_columns=None) -> SpriteSheet:
    return SpriteSheet(size, max_columns).new()

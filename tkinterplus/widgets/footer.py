...
class Footer:
    def __init__(self, master: tkinter.Tk = None, bg: str = None, **kw):
    ...
    def _geo(self) -> None:
        ...
        if self.manager == GRID:
            ...
            self._container.grid(
                row=self._row, column=0, columnspan=self._column + 1, sticky="ews"
            )
    ...
    def add_button(self, text: str = None, **kw) -> None:
    ...
    def remove_button(self, index) -> None:
        """Delete a button from the footer"""
        self.buttons[index].destroy()

    def configure_button(self, index: int, **kw) -> None:

...
class Form(tkinter.Frame):
    def __init__(self, parent: tkinter.Tk, title=None, description=None, tearoff=False):
        ...
        self.column = 0
        if title:
            tkinter.Label(self, text=title, font=("bold", 12)).grid(
                row=0, column=0, sticky=tkinter.W
            )
            self._size(1)
            if description:
                tkinter.Label(self, text=description).grid(
                    row=1, column=0, sticky=tkinter.W
                )
                self._size(1)

    def _size(self, row: int = None, colum: int = None) -> tuple[int, int]:
    ...
    def _button(self, name, command=None) -> None:
        """Internal function"""
        r, c = self._size()
        tkinter.Button(self, text=name, command=command).grid(
            row=r, column=c, sticky=tkinter.W
        )
        self._size(1)

    def _label(self, text, font=None) -> None:
        """Internal function"""
        r, c = self._size()
        tkinter.Label(self, text=text, font=font).grid(
            row=r, column=c, sticky=tkinter.W
        )
        self._size(1)

    def _upload(self, multiple, size, **kw) -> None:
        """Internal function"""
    ...
    def add_radio(self, question, *options) -> None:
        """Add multiple choice"""
        ...
        for option in options:
            r, c = self._size()
            tkinter.Radiobutton(self, text=option, variable=tmp, value=count).grid(
                row=r, column=c, sticky=tkinter.W
            )
            count += 1
            self._size(1)

    def add_checkbox(self, question, *options) -> None:
        """Add checkbox"""
        self._label(question)
        for option in options:
            r, c = self._size()
            tkinter.Checkbutton(self, text=option).grid(
                row=r, column=c, sticky=tkinter.W
            )
            self._size(1)

    def add_dropdown(self, question, *options) -> None:
        """Add dropdown list"""
        self._label(question)
    ...
    def add_file(self, question, filetypes=None, multiple=False, maxsize=None):
        ...
        if isinstance(filetypes, list):
            ...
            elif isinstance(filetypes[0], tuple):
                filetype = filetypes
        elif isinstance(filetypes, str):
            filetype = [("Custom Files", convert(filetypes)), ("All files", "*.*")]
        self._button(
            "Add file", lambda: self._upload(multiple, maxsize, filetypes=filetype)
        )
        self._size(1)

    def add_linear_scale(self, question, from_=1, to=5, start=None, end=None) -> None:
        """Add linear scale"""
        self._label(question)
        r, c = self._size()
        ...
        count = from_
        for i in range(to):
            tkinter.Label(scale, text=str(count), justify=tkinter.CENTER).grid(
                row=0, column=i + 1
            )
            tkinter.Radiobutton(
                scale, variable=SCALE, value=count, justify=tkinter.CENTER
    ...
    def add_radio_grid(self, question, row, column) -> None:
        ...
        for rr in row:
            countcc = 0
            for cc in column:
                tkinter.Radiobutton(grid, variable=values[countrr], value=countcc).grid(
                    row=countrr + 1, column=countcc + 1
                )
                countcc += 1
            countrr += 1
    ...
    def add_checkbox_grid(self, question, row, column) -> None:
        """Add checkbox grid"""
        self._label(question)
        r, c = self._size()
        ...
        for rr in row:
            ...
            for cc in column:
                tkinter.Checkbutton(
                    grid, variable=values[total], onvalue=True, offvalue=False
                ).grid(row=countrr + 1, column=countcc + 1)
                countcc += 1
                total += 1
            countrr += 1
    ...
    def add_short_answer(self, question) -> None:
        """Add short answer"""
        self._label(question)
        r, c = self._size()
    ...
    def add_paragraph(self, question) -> None:
        """Add paragraph"""
        self._label(question)
        r, c = self._size()
        # Label(self,text=question).grid(row=r,column=c,sticky=W)
        tkinter.Text(self, width=16, height=1).grid(
            row=r + 1, column=c, sticky=tkinter.W
        )
        self._size(2)

    def add_title(self, title, description=None) -> None:
    ...
    def add_image(self, file, title=None, **kw) -> None:
        """Add image"""
        if title:
            self._label(title)
        r, c = self._size()
        Picturebox(self, file, **kw).grid(row=r, column=c, sticky=tkinter.W)
        self._size(1)

    def add_submit_button(self, command) -> None:
        """Add submit button"""
        self._button("Submit", command)

    def add_reset_button(self, command) -> None:
        """Add reset button"""

...
__all__ = ["SLOW", "FAST", "Tween", "fx", "opacity_color", "Animation"]
...
class Tween:
    def __init__(self, anim, stepcommand: Callable, callback: Callable, **kw):
    ...
    def __call__(self):
        ...
        if self.stepcommand != None:
            self.stepcommand(self.__now, self.anim)
        self.callback(**self.kw)

    def c(self):  # TODO: remove
        self()
...
class fx:
    def __init__(
        ...
        queue: bool = None,
        easing: str = None,
        stepcommand: Callable = None,
        progresscommand: Callable = None,
        donecommand: Callable = None,
        name: str = None,
        prop: str = None,
    ):
        self.anim = anim
    ...
    def configure(self, **kw) -> None:
        if self.locked == False:
            if "duration" in kw and kw["duration"] != None:
                self.duration = kw["duration"]
            if "TagOrId" in kw and kw["TagOrId"] != None:
                self.TagOrId = kw["TagOrId"]
            if "queue" in kw and kw["queue"] != None:
                self.queue = kw["queue"]
            if "easing" in kw and kw["easing"] != None:
                self.easing = kw["easing"]
            if "stepcommand" in kw and kw["stepcommand"] != None:
                self.stepcommand = kw["stepcommand"]
            if "progresscommand" in kw and kw["progresscommand"] != None:
                self.progresscommand = kw["progresscommand"]
            if "donecommand" in kw and kw["donecommand"] != None:
                self.donecommand = kw["donecommand"]
            if "name" in kw and kw["name"] != None:
                self.name = kw["name"]
            if "prop" in kw and kw["prop"] != None:
                self.prop = kw["prop"]
        else:
    ...
    def step(self) -> None:
        ...
        if self.stepcommand != None:
            self.stepcommand(self.__now, self)
    ...
    def _number(self, option: str, start: int, end: int, callback: Callable) -> None:
        _start = start
        _end = end
        ...
        for i in easing(self.easing, _start, _end):
            ...
            self.widget.after(
                round(t),
                lambda a=self.anim, s=self.stepcommand, c=callback, kw=kw: Tween(
                    a, s, c, **kw
                ).c(),
            )

            if i + 1 == _end:
                self.widget.after(round(t) + 1, self.done)
            t += time

    def _color(self, option: str, start: str, end: str, callback: Callable) -> None:
    ...
    def _special(self, option: str, start: int, end: int, callback: Callable) -> None:
    ...
    def _widget_number(self, option: str, start: int, end: int) -> None:
        self._number(option, start, end, self.widget.configure)

    def _widget_color(self, option: str, start: str, end: str) -> None:
        self._color(option, start, end, self.widget.configure)

    def _widget_special(self, option: str, start: int, end: int) -> None:
        self._special(option, start, end, self.widget.place)

    def __run_widget(self) -> None:
        number = [
            "bd",
            "border",
            "borderwidth",
        ...
        special = ["x", "y"]
        if self.prop in number:
            self._widget_number(self.prop, int(str(self.start)), int(str(self.end)))
        elif self.prop in color:
            self._widget_color(self.prop, self.start, self.end)
        elif self.prop in special:
            self._widget_special(self.prop, self.start, self.end)
        else:
            print('Unknown option "%s"' % (self.prop))
        return self

    def _item_number(self, option: str, start: int, end: int) -> None:
        self._number(option, start, end, self.widget.configure)

    def _item_color(self, option: str, start: str, end: str) -> None:
        self._color(option, start, end, self.widget.configure)

    def _item_special(self, option: str, start: int, end: int) -> None:
        self._special(
            option,
            start,
            end,
            lambda tag=self.TagOrId, **kw: self.widget.moveto(tag, **kw),
        )

    def __run_item(self) -> None:
        number = []
        color = ["fill", "activefill"]
        special = ["x", "y", "width", "height"]
        if self.prop in number:
            self._item_number(self.prop, int(str(self.start)), int(str(self.end)))
        elif self.prop in color:
            self._item_color(self.prop, self.start, self.end)
        elif self.prop in special:
            self._item_special(self.prop, self.start, self.end)
        else:
            print('Unknown option "%s"' % (self.prop))
        if self.stepcommand != None:
            self.stepcommand(self.__now, self)
    ...
    def run(self) -> None:
        """Run the fx"""
        self.locked = True
        if self.progresscommand != None:
            self.progresscommand(
                self.anim, self.anim.__progress, self.anim.__remainingms
            )
        if self.TagOrId == None:
            return self.__run_widget()
        else:
            return self.__run_item()
        # self.progresscommand()


def opacity_color(opacity: str, color1: str, color2: str) -> str:
    """
...
class Animation:
    def __init__(self, widget: tkinter.Widget, TagOrId: str = None):
        ...
        self.configure(widget=widget, TagOrId=TagOrId)
    ...
    def configure(self, **kw) -> None:
        ...
        if "TagOrId" in kw and kw["TagOrId"] != None:
            if isinstance(self.widget, tkinter.Canvas):
                self.TagOrId = kw["TagOrId"]
            else:
                raise TkinterPlusError(
                    'WIDGET must be tkinter.Canvas to use TAGORID but got "%s"'
    ...
    def toggle(
        ...
        """
        if self.visable:
            self.hide(duration, easing, donecommand)
        else:
            self.show(duration, easing, donecommand)
        return self
        ...
        **kw,
        ...
        for prop in kw:
            ...
            if self.TagOrId == None:  # Widget
                match prop:
                    case "x":
                        item.start = self.widget.winfo_x()
                    case "y":
                        item.start = self.widget.winfo_y()
                    case _:
                        item.start = self.widget.cget(prop)  # Save default kw
            else:  # Canvas Item
                match prop:
                    case "x":
    ...
    def clear_queue(self, name: str = "fx") -> None:
    ...
    def delay(self, duration: int, name: str = "fx") -> None:
    ...
    def dequeue(self, name: str = "fx") -> None:
    ...
    def finish(self, queue: str = "fx") -> None:
    ...
    def queue(self, name: str = "fx") -> None:
    ...
    def stop(
        self, name: str = "fx", stop_queue: bool = False, jump_to_end: bool = False
    ) -> None:
    ...
    def fade_toggle(
        ...
        """
        if self.visable:
            self.fade_out(duration, easing, queue, donecommand)
        else:
            self.fade_in(duration, easing, queue, donecommand)
        return self
    ...
    def slide_down(
        ...
        print("SLIDE DOWN")
        if donecommand == None:
            donecommand = lambda e: self.show()
        self.animate(
            duration=duration,
            easing=easing,
            queue=queue,
    ...
    def slide_up(
        ...
        print("SLIDE UP")
        if donecommand == None:
            donecommand = lambda e: self.hide()
        self.animate(
            duration=duration,
            easing=easing,
            queue=queue,
    ...
    def slide_toggle(
        ...
        """
        if self.visable:
            self.slide_down(duration, easing, queue, donecommand)
        else:
            self.slide_up(duration, easing, queue, donecommand)
        return self
